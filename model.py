...
class MemN2N(object):
    def __init__(self, config, sess):
        ...
        self.input = tf.placeholder(tf.float32, [None, self.edim], name="input")  # hidden vector
        self.time = tf.placeholder(tf.int32, [None, self.mem_size], name="time")  # memory
    ...
    def build_memory(self):
        ...
        self.A = tf.Variable(tf.random_normal([self.nwords, self.edim], stddev=self.init_std))  # A \in R^{d x V}
        self.B = tf.Variable(tf.random_normal([self.nwords, self.edim], stddev=self.init_std))  # B \in R^{d x V
        ...
        Ain = tf.add(Ain_c, Ain_t)  # Ain dim: [batch_size, mem_size, edim]
        ...
        for h in xrange(self.nhop):
            ...
            Dout = tf.add(Cout, Bout2dim)  # dim: [batch_size, edim]
